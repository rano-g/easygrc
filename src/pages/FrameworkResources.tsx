...
const FrameworkResources = () => {
  ...
  const handleResourceDownload = (resource: FrameworkResource) => {
    if (resource.type === 'checklist') {
      // Generate and download custom checklist
      const filename = `${resource.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.xlsx`;
      ChecklistGenerator.downloadChecklist(resource.id, filename);
      console.log(`Generating and downloading checklist: ${resource.title}`);
